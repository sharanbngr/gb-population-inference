...
def generate_noise_realization(
    dt, Tobs, noisemodel: LISAModel = lisa_models.sangria, seed=0, DEBUG=False
):
    Nt = Tobs // dt
    Nchannels = 3
    ...
    retdict = {
        "expected_PSD": np.zeros((Nchannels, N)),
        "fft": np.zeros((Nchannels, N), dtype=np.complex128),
        "freqs": freqs,
        "timeseries": np.zeros((Nchannels, Nt)),
    }
    ...
    if DEBUG:
        ...
        f, psdA = scipy.signal.welch(
            x_new[0, :], fs=1 / dt, nperseg=len(x_new[0]), window="boxcar", noverlap=0
        )
        f, psdT = scipy.signal.welch(
            x_new[2, :], fs=1 / dt, nperseg=len(x_new[0]), window="boxcar", noverlap=0
        )
