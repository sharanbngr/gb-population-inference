...
class SinosoidWaveform(AbstractWaveform):
    ...
    def compute_waveform_parameters(self, parameters, in_place=True):
        amplitude_args = {
            ...
            if col in get_function_args(self.amplitude_model)
        }
        frequency_args = {
            col: parameters[col]
            for col in parameters.keys()
            if col in get_function_args(self.frequency_model)
        }
        phase_args = {
            col: parameters[col]
            for col in parameters.keys()
            if col in get_function_args(self.phase_model)
        }
    ...
    def generate_waveforms(
        self,
        parameters: Dict[str, Union[float, np.array]],
        sample_rate=0.25,
        duration=10000,
        summed=False,
    ):
        ...
        if all(isinstance(variable, float) for variable in (A, f, phase)):
            ...
            if summed:
                self.timeseries = np.zeros(len(ts))
                for i in tqdm(range(len(A))):
                    self.timeseries += A[i] * np.sin((2 * np.pi * f[i] * ts) + phase[i])
            else:
